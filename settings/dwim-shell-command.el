...
Set TEMP-DIR to a unique temp directory to this template."
  ...
  (setq template (replace-regexp-in-string "\\(\<\<fnd\>\>\\)" (file-name-nondirectory file) template nil nil 1))

  (setq template (replace-regexp-in-string "\\(\<\<fnde\>\>\\)" (file-name-sans-extension (file-name-nondirectory file)) template nil nil 1))
